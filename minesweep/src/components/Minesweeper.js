...
const Minesweeper = ({diff, newGame, setNewGame , setStatus, setResult, t}) => {
    ...
    }, [nrows, ncols, nmarked, exploded, flagin, diff, newGame ]);
    ...
    function visual ()
    {
        ...
        for (let z = 0; z < v.length; z++){
            for (let i = 0; i < v[z].length; i++) {
                ...
                if(v[z][i] === "H")
                {
                    d.push(<button key={counter} onClick={(e) => clickin(e,z,i)} onContextMenu={(e) => contextMenu(e,z,i)}></button>)
                }
                else if (v[z][i] === "F")
                {
                    d.push(
                        <button key={counter} onClick={(e) => clickin(e,z,i)} onContextMenu={(e) => contextMenu(e,z,i)}>
    ...
    function flagState()
    {
        setFlagin(flagin => !flagin);
    }
    function flagButton()
    {
        if(flagin)
        {
            return <button onClick={()=>flagState()}>You are in flag mode</button>;
        }
        else
            return <button onClick={()=>flagState()}>You are in Uncover Mode</button>;
    }
